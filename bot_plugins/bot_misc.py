...
class Misc:
    ...
    def initialize(self):
        txt_cmds = {
            # self.help: ['doc', 'docs'],#, 'help'], Disabled
        ...
        msg_cmds = {
            'Stop spam': self.stop_spam_msg_cmd
        }
        ...
        self.spam_stop_ids = set()

        return txt_cmds, None, user_cmds, msg_cmds
    ...
    async def salute(self,
        ...
        if user is None:
            user = ctx.author
        txts = ['Hey {user}!', 'Hello {user}!', 'Good day to you, {user}!',
    ...
    async def spam(self,
        ...
        for i in range(amount):
            if log_count:
                pre = f'{i+1}: '
            try:
                for msg_id in self.spam_stop_ids:
                    if msg_id in sent_ids:
                        await ctx.send(f'Aborted, {i+1}/{amount} msgs sent')
                        self.spam_stop_ids.remove(msg_id)
                        return

                if not responded:
                    interaction = await ctx.respond(f'{pre}{content}')
                    responded = True
                    msg = interaction.message
                else:
                    msg = await ctx.send(f'{pre}{content}')

                if msg is not None:
                    sent_ids.add(msg.id)
                else:
                    logger.warn(
                        f'Error in spam: No msg id for interaction in channel {ctx.channel}')
    ...
    async def stop_spam(self, ctx):
        "Stop spamming a message:\nJust reply to one of the spam message (Admin only!)"
        ...
        if ctx.message is None:
            await ctx.respond('This command cannot be used as a slash command!\nPlease use it as a text command.')
            return

        if ctx.message.type != discord.MessageType.reply or ctx.message.reference is None:
            await ctx.respond('You must reply to a message being spammed!')
            return

        target = ctx.message.reference
        # if target.author.id != ctx.me.id:
        # 	await ctx.respond('You must reply to a message sent by Helium!')
        # 	return

        self.spam_stop_ids.add(target.message_id)

    async def stop_spam_msg_cmd(self, ctx, msg):
        if not self.is_admin(ctx.author):
            await ctx.respond(f'Only admins can use this command!')
            return
        
        if msg.author.id == self.user.id:
           self.spam_stop_ids.add(msg.id)
           await ctx.respond('Spam stopped!')
        else:
            await ctx.respond('Sorry, but I cannot stop this guy...')
    ...
    async def help(self,
        ...
        # Plugin help page
        p = filter(lambda p: p.__doc__.split(':')[
                   1].strip() == cat, self.plugins)
        p = next(p, None)
    ...
    async def mention(self,
        ...
        # 	return
        try:
            await ctx.replace(user.mention)
