...
class Moderation:
    ...
    def get_block_delay(self, arg):
        ...
        unit_conv = {k: v for v, keys in unit_conv.items()
                     for k in keys}  # Reversed dict
    ...
    def get_banned_text(self, user, embed=False):
        if user.id in self.banned_list:
            ...
            if delay is None:
                ...
                days, hours, mins, secs = list(
                    map(int, (days, hours, mins, secs)))
                if days > 0:
    ...
    async def delete(self,
        ...
        if not self.is_admin(ctx.author):
            await ctx.respond('Only the admins can use this command!')
            return
        ...
        if from_user is not None:
            def check(
                e): return e.id != ctx.message.id and e.author.id == from_user.id
        else:
            def check(e): return e.id != ctx.message.id
    ...
    async def set_status(self, msg, *args):
        ...
        a = msg.channel.send(
            f"Settings status to {args[0]} - {status_list[args[0]]}")
    ...
    async def set_activity(self, msg, *args):
        ...
        txts = {txt: (act, data) for act, data in activity_list.items()
                for txt in data['txt']}
